...
set BASE_DIR=E:\research\data\Animation\Vertex-Edge
@REM Generator will append _seed + random number to output directory name
@REM To create subdirectories under BASE_DIR, output directory should be BASE_DIR\VE
@REM Generator will create BASE_DIR\VE_seed + random number
set OUTPUT_DIR=%BASE_DIR%\VE
...
:: 使用唯一的日志文件名避免文件锁定问题
set LOG_FILE=generation_log_VE_!ATTEMPT_COUNT!.txt

:: 运行生成器并捕获输出
echo 正在运行generator...
xmake r generator -t 4 -o "%OUTPUT_DIR%" > "!LOG_FILE!" 2>&1
