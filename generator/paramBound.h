...
using Matrix3r = Eigen::Matrix<Rational, 3, 3>;
...
public:
	...
	RecParamBound(Array2r const &p1 = Array2r(0,0), Array2r const &p2 = Array2r(1,1)) :
		pMin { std::min(p1[0], p2[0]), std::min(p1[1], p2[1]) },
		pMax { std::max(p1[0], p2[0]), std::max(p1[1], p2[1]) } {
	}

	Array2r operator[](int i) const { return i == 0 ? pMin : pMax; }

	RecParamBound operator&(RecParamBound const &o) const {
		RecParamBound ret;
		ret.pMin = pMin.cwiseMax(o.pMin);
		ret.pMax = pMax.cwiseMin(o.pMax);
		return ret;
	}

	bool isDegenerate() const { return pMin.x() > pMax.x() || pMin.y() > pMax.y(); }
	bool isInside(Array2r const &o) const { return (o.x() >= pMin.x() && o.x() <= pMax.x() && o.y() >= pMin.y() && o.y() <= pMax.y()); }

	Array2r diagonal() const { return pMax - pMin; }
	Array2r corner(int i) const { return Array2r((*this)[(i & 1)][0], (*this)[(i & 2) ? 1 : 0][1]); }

	RecParamBound interpSubpatchParam(const int id) const {
		Array2r pMid = (pMin + pMax)/2;
		return RecParamBound(corner(id), pMid);
	}

	Array2r centerParam() const { return (pMin + pMax)/2; }
...
public:
	...
	BaryCoord(const Rational _u, const Rational _v): u(_u), v(_v), w(Rational(1)-_u-_v) {}
	BaryCoord(const Rational _u, const Rational _v, const Rational _w): u(_u), v(_v), w(Rational(1)-_u-_v) {}
	BaryCoord(const Array2r& bc): u(bc[0]), v(bc[1]), w(Rational(1)-bc[0]-bc[1]) {}
	BaryCoord(const BaryCoord& bc): u(bc.u), v(bc.v), w(Rational(1)-bc.u-bc.v) {}
	// BaryCoord(const double u, const double v, const double w): u(u), v(v), w(w) {}
	// BaryCoord(const BaryCoord& bc): u(bc.u), v(bc.v), w(bc.w) {}
	BaryCoord operator*(const Rational scalar) const {
...
public:
	...
	Array2r centerParam() const { return Array2r((nodes[0].u+nodes[1].u+nodes[2].u)/Rational(3), (nodes[0].v+nodes[1].v+nodes[2].v)/Rational(3)); }
	Rational width() const {
		Rational uMax=std::max(std::max(nodes[0].u, nodes[1].u), nodes[2].u), uMin=std::min(std::min(nodes[0].u, nodes[1].u), nodes[2].u); 
		Rational vMax=std::max(std::max(nodes[0].v, nodes[1].v), nodes[2].v), vMin=std::min(std::min(nodes[0].v, nodes[1].v), nodes[2].v); 
